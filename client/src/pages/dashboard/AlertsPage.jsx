...
export default function AlertsPage() {
  ...
  return (
    <div>
      <h2 className="font-display text-xl font-bold mb-6">{t("dashboard.alerts")}</h2>
      {alerts.length === 0 ? (
            ...
            <div key={alert.id} className="flex items-center gap-4 p-4 border border-border rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-2 items-center mt-1">
                  <span className="text-xs text-muted-foreground">{t("product.originalPrice")}: <span className="font-mono text-[hsl(var(--accent))]">${alert.target_price?.toFixed(2)}</span></span>
                  <span className="text-xs text-muted-foreground">{t("product.outletPrice")}: <span className="font-mono">${alert.current_price?.toFixed(2)}</span></span>
                </div>
              </div>
