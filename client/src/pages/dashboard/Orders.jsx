            ...
            <div key={order.id} className="p-4 lg:p-6 border border-border rounded-xl hover:border-muted-foreground/30 transition-colors">
              <div className="flex flex-col sm:flex-row justify-between gap-3">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge variant="secondary" className={STATUS_STYLES[order.status]}>{order.status?.replaceAll("_"," ")}</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">{moment(order.createdAt).format("MMM D, YYYY")}</p>
                  <p className="text-xs text-muted-foreground mt-1">{order.items?.length || 0} item(s)</p>
                  {order.carrier ? <p className="text-xs text-muted-foreground mt-1">Carrier: {order.carrier}</p> : null}
                </div>
                  ...
                  {order.timeline.map((step) => (
                    <div key={step.key} className={`rounded-lg border px-3 py-2 ${step.status === "completed" ? "border-[hsl(var(--accent))]/30 bg-[hsl(var(--accent))]/5" : "border-border bg-secondary/20"}`}>
                      ...
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        {step.happenedAt ? moment(step.happenedAt).format("MMM D, YYYY") : "Pending"}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              {order.trackingUrl || order.shipmentNotes ? (
                <div className="mt-4 rounded-lg bg-secondary/30 px-4 py-3 text-xs text-muted-foreground">
                  {order.trackingUrl ? (
                    <p>
                      Tracking URL: <a href={order.trackingUrl} target="_blank" rel="noreferrer" className="text-[hsl(var(--accent))] underline underline-offset-4">{order.trackingUrl}</a>
                    </p>
                  ) : null}
