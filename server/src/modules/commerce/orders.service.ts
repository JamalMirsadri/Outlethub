...
import { multiLevelReferralPointsService } from "./multi-level-referral-points.service.js";
...
function mapOrder(order: OrderWithRelations) {
  return {
    ...
    agentCostAmount: toNumber(order.agentCostAmount),
...
export class OrdersService {
  ...
  public async createOrderFromCart(input: {
    ...
    const order = await prisma.$transaction(async (transaction) => {
      const createdOrder = await transaction.order.create({
        data: {
          ...
          agentCostAmount: totals.agentCostAmount,
    ...
    await multiLevelReferralPointsService.syncMultiLevelReferralPoints(persistedOrder.id);
  ...
  public async updateOrderStatus(orderId: string, status: OrderStatus) {
    ...
    await multiLevelReferralPointsService.syncMultiLevelReferralPoints(order.id);
  ...
  public async refundOrder(orderId: string, amount?: number | null, internalNotes?: string | null) {
    ...
    await multiLevelReferralPointsService.syncMultiLevelReferralPoints(refunded.id);
